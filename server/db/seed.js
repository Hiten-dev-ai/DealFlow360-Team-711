...
try {
  ...
  const tiers = [['71110000-0000-4000-8000-000000000001','Standard',20],['71110000-0000-4000-8000-000000000002','Strategic',5]];
  ...
  for(const [tierIndex,tier] of tiers.entries())for(const [categoryIndex,category] of categories.entries()){const ceilings=[[1000,800,500],[1500,1200,1000]][tierIndex][categoryIndex];await client.query(`INSERT INTO discount_policies(workspace_id,tier_id,category_id,ceiling_bps) VALUES($1,$2,$3,$4) ON CONFLICT(workspace_id,tier_id,category_id) DO UPDATE SET ceiling_bps=EXCLUDED.ceiling_bps`,[workspaceId,tier[0],category[0],ceilings]);}
  ...
  const customers=[['71140000-0000-4000-8000-000000000001',tiers[1][0],'Acme Corp','deals@acme.example','Acme Corp',0],['71140000-0000-4000-8000-000000000002',tiers[0][0],'Beta Industries','buyer@beta.example','Beta Industries',7500000],['71140000-0000-4000-8000-000000000003',tiers[0][0],'Northstar Labs','ops@northstar.example','Northstar Labs',0],['71140000-0000-4000-8000-000000000004',tiers[0][0],'Delta Retail','procurement@delta.example','Delta Retail',2500000]];
  for(const row of customers)await client.query(`INSERT INTO customers(id,workspace_id,tier_id,name,email,company,overdue_minor) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,email=EXCLUDED.email,overdue_minor=EXCLUDED.overdue_minor`,[row[0],workspaceId,...row.slice(1)]);
