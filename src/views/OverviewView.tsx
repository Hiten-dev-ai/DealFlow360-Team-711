...
export function OverviewView({ onNavigate }: { onNavigate: (view: AppView) => void }) {
  ...
  return <div className="page-stack"><section className="page-heading"><div><span className="page-kicker">Sales operations</span><h2>Sales dashboard</h2><p>Track the path from quotation to payment.</p></div><button type="button" className="primary-action" disabled={connection !== 'online'} onClick={() => onNavigate('quotations')}>New quotation <ArrowUpRight size={17} /></button></section><section className="metric-grid" aria-label="Workspace metrics">{metrics.map(({ label, value, note, icon: Icon }) => <article className="metric-card" key={label}><span className="metric-icon"><Icon size={19} /></span><span>{label}</span><strong>{value}</strong><small>{note}</small></article>)}</section><section className="flow-card"><div className="flow-heading"><span><Route size={19} /></span><div><h3>Core deal flow</h3><p>Open any stage to continue the workflow.</p></div></div><div className="flow-steps">{flow.map((step, index) => <button type="button" className="flow-step" key={step.label} onClick={() => onNavigate(step.view)}><span>{index + 1}</span><strong>{step.label}</strong><ArrowUpRight size={14} /></button>)}</div></section></div>;
}
